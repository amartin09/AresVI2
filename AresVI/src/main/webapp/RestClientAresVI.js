...
// Model for Usuarios entity
    models.Usuarios = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/AresVI/webresources/com.aresvi.aresvi.usuarios/",
        defaults: {
            usuario: "",
            contrasenia: "",
            habilitado: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('usuario');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8084/AresVI/webresources/com.aresvi.aresvi.usuarios/';
            }
    ...
    // Collection class for Usuarios entities
    models.UsuariosCollection = Backbone.Collection.extend({
        model: models.Usuarios,
        url: "http://localhost:8084/AresVI/webresources/com.aresvi.aresvi.usuarios/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.UsuariosCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-usuarios-list-itemis template identifier for item
                        templateName: '#tpl-usuarios-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.usuarios = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.usuarios,
                    // tpl-usuarios-details is template identifier for chosen model element
                    templateName: '#tpl-usuarios-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Usuarios();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-usuarios-details is a template identifier for chosen model element
                templateName: '#tpl-usuarios-details',
